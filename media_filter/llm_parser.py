...
5. semantic_query — то, что НЕ свелось к структуре. Любое описание ТЕМАТИКИ, СМЫСЛА, СОДЕРЖАНИЯ подписи (что изображено, о чём текст, упоминания людей/событий/тем/категорий типа «еда», «митинг», «погода», «спорт» и т.п.) — это semantic_query. Если в запросе чисто числовые/именные ограничения (размер, длительность, явное имя файла, отправитель по @юзернейму) — semantic_query = null.

6. needs_semantic_rerank = true тогда и только тогда, когда semantic_query != null.

7. sender_username — без "@". Заполняется ТОЛЬКО когда пользователь явно указал юзернейм с @ или сказал «от user X». Имя/фамилия без юзернейма — НЕ sender_username.

8. mime_type_contains и file_name_contains — ТЕХНИЧЕСКИЕ метаданные файла, НЕ его содержимое:
     • file_name_contains: подстрока в ИМЕНИ ФАЙЛА (например, "report-2024.pdf"). Примеры: "файлы с report-*" → "report-". "PDF которые содержат budget" → "budget".
     • mime_type_contains: тип MIME ("pdf", "image", "video/mp4").
   ВАЖНО: смысловые слова о содержимом, теме, событии (например, "ужин", "обед", "еда", "митинг", "пожар", "погода", "котики") НИКОГДА не идут в эти поля. Они описывают О ЧЁМ сообщение, а не имя файла. Такие слова ВСЕГДА идут в semantic_query.
   У фото, видео, голосовых, кружков и URL-сообщений в Telegram обычно нет file_name — фильтр по file_name_contains для них почти всегда вернёт ноль результатов, поэтому даже сомнительные кандидаты лучше отправлять в semantic_query.

9. Никаких комментариев, markdown-блоков, скобок-постамбул. Только валидный JSON-объект.

ПРИМЕРЫ ПРАВИЛЬНОГО ПАРСИНГА:

  USER_QUERY: "подпись у картинки 'ужин' или 'обед' или любой другой прием пищи"
  → structured_filters: всё null. semantic_query: "подпись о еде, приёме пищи (ужин, обед и т.п.)". needs_semantic_rerank: true.

  USER_QUERY: "видео больше 10 МБ"
  → structured_filters.file_size_min_bytes: 10485760. semantic_query: null. needs_semantic_rerank: false.

  USER_QUERY: "видео про митинг от @ivanov больше 5 минут"
  → file_size: null. duration_min_sec: 300. sender_username: "ivanov". semantic_query: "митинг". needs_semantic_rerank: true."""
